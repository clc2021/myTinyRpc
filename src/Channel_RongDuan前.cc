...
#include "./include/rpc_header.pb.h"
...
bool MprpcChannel::GetServiceAddress(const std::string& service_name, // eg. UserServiceRpc
    ...
    for (int i = 0; i < host_datas.size(); i++) {
        ...
        service_address[i].ip = host_data.substr(0, idx); // 127.0.0.1
        service_address[i].port = atoi(host_data.substr(idx+1, host_data.size()-idx).c_str()); // 2181
    }
    return true;
}
