...
void ZkClient::Create(const char *path, const char *data, int datalen, int state)
{
	...
	if (ZNONODE == flag) // 表示path的znode节点不存在
	{
		...
		// zoo_create(m_zhandle, parentPath, nullptr, 0, &ZOO_OPEN_ACL_UNSAFE, 0, nullptr, 0);
...
// 根据指定path，获取子节点的值
std::vector<std::string> ZkClient::GetChildData(const char *path) { 
	
	struct String_vector buffer;
	//int zoo_get_children(zhandle_t *zh, const char *path, int watch, struct String_vector *strings);
	int flag = zoo_get_children(m_zhandle, path, 0, &buffer);
	if (flag == ZOK) {
		// 成功了
		for (int i = 0; i < buffer.count; i++) {
			std::string path_string(path);
			std::string temp = path_string + "/" + buffer.data[i];
			m_childs.push_back(temp);
		}
		

	} else {
		std::cout << "znode get childs error... path:" << path << std::endl;
		exit(EXIT_FAILURE);
	}
	return m_childs;
}
...
// 创建具有多个子节点的父节点
void ZkClient::CreateMultipleChildren(const char *path, const char * childPath, const char *data, int datalen, int state)
{
	// 确定已经是创建了父节点
	char path_buffer[128];
    int bufferlen = sizeof(path_buffer);
	std::string ChildPath = std::string(path) + "/" + childPath;
	int flag;
	...
	std::cout << "ChildPath = " << ChildPath << std::endl << std::endl;
	
	flag = zoo_exists(m_zhandle, ChildPath.c_str(), 0, nullptr);
	if (ZNONODE == flag) // 表示path的znode节点不存在
	{
		// flag = zoo_create(m_zhandle, path, data, datalen,&ZOO_OPEN_ACL_UNSAFE, state, path_buffer, bufferlen);
		flag = zoo_create(m_zhandle, ChildPath.c_str(), data, datalen, &ZOO_OPEN_ACL_UNSAFE, state, path_buffer, bufferlen);
		if (flag == ZOK)
		{
			std::cout << "child znode create success... path:" << ChildPath << std::endl;
		}
		else
		{
			std::cout << "flag:" << flag << std::endl;
			std::cout << "child znode create error... path:" << ChildPath << std::endl;
			exit(EXIT_FAILURE);
		}
	}
}
