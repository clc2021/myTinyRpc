...
public class LimitAspect {
    ...
    public Object annotationAround(ProceedingJoinPoint jp) {
        ...
        // 没有配置QPS则使用统一限流配置
        if (limitingStrategy.QPS() == 0){
            LimitingRule unifiedLimitConfig = threadLocal.get();
            threadLocal.remove();
            if (unifiedLimitConfig.getLimitValue() == null || unifiedLimitConfig.getLimitValue().equals("")) {
                log.error("统一限流配置LimitKey为空，本次限流无效，请检查全局配置是否正确！");
                pass = true;
            }
            else {
                pass = limitProcess.limitHandle(unifiedLimitConfig);
            }
            if (!pass){ // 没通过
                if (unifiedLimitConfig.getFallBackMethod() != null){
                    return unifiedLimitConfig.getFallBackMethod().get(); // 获取降级方法
                }
            }else {
                try {
                    return jp.proceed(); // 通过了执行jp.proceed()执行之后的代码
        ...
        // 如果limitKey不为空则对limitKey对应的属性进行限流处理
            ...
            String[] parameterNames = ((CodeSignature) jp.getSignature()).getParameterNames(); // 方法的参数名
            Object[] args = jp.getArgs(); // 方法的参数值
